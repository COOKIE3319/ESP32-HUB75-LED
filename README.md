...
- **手动 HUB75 驱动**：直接寄存器操作，采用关输出→移位→锁存→显示的标准时序，提升色彩准确性
- **双核架构**：Core 1 独占 LED 扫描，Core 0 运行 WiFi + Web 服务器
- **Gamma 校正**：默认 γ=1.0（避免对 sRGB 图片暗部二次压暗）
- **6位量化优化**：上传后按面板6位色深四舍五入量化，并保留非零暗部细节
- **亮度控制**：通过 `BRIGHTNESS` 宏按 BCM 位平面时序缩放亮度 (0-255)
- **实时参数调节**：支持通过 `/settings` 接口在线调节 Gamma，并立即重算当前图片
- **WiFi 自动重连**：断线自动恢复
...
| GET | `/settings` | 获取显示参数（Gamma范围与当前值） |
| POST | `/settings` | 设置显示参数（`gamma`） |
